...
```
Based on this new data set, a visualization of missing data was created which showed that there were over 100 missing observations for seed count. 
```{r}
#visualizing the missing data
gg_miss_var(seed)
```
** Figure 4. Missing Data Visualization** Each column is displayed on the y-axis with the number of missing variables on the x-axis, displaying that the column for number of seeds has missing data.
...
seed_subset <- seed %>% 
  drop_na(nr_seeds)
```
To explore relationships between predictor variables in our new data set, we created a pairs plot rather than a Pearson's r correlation plot because two out of the three predictors are categorical. Based on the plot, none of the predictor variables appear to have a strong relationship with another that could induce colinearity. 
```{r}
#using data subset
seed_subset %>% 
  #selecting all variables
  select(treatment: total_nr_infl) %>% 
  #creating plot
  ggpairs()
```
**Figure 5: Relationships Between Predictor Variables** Each plot in the grid compares two predictor variables on the x and y axis and creates a visualization.

Now that the data has been organized, clean from missing values, and explored we can now create null and full models to predict seed count and use for analysis. The null model includes no predictors for seed count, whereas the full model will include all three predictor variables and will later be used to assess how seed count varies for each predictor. 
```{r}
#creating models for both null and full
null <- lm(nr_seeds ~ 1, data = seed_subset)
full <- lm(nr_seeds ~ species + treatment + total_nr_infl, data = seed_subset)
```
